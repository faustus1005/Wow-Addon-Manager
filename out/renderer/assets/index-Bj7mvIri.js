...
function reducer(state, action) {
  switch (action.type) {
    ...
    case "SET_INSTALLED_ADDONS": {
      const updateCount = action.addons.filter((a2) => a2.updateAvailable && !a2.isIgnored).length;
      return { ...state, installedAddons: action.addons, updateCount };
    }
    case "UPSERT_ADDON": {
      const list = state.installedAddons.filter((a2) => a2.id !== action.addon.id);
      const addons = [...list, action.addon].sort((a2, b) => a2.name.localeCompare(b.name));
      const updateCount = addons.filter((a2) => a2.updateAvailable && !a2.isIgnored).length;
      return { ...state, installedAddons: addons, updateCount };
    }
    case "REMOVE_ADDON": {
      const addons = state.installedAddons.filter((a2) => a2.id !== action.addonId);
      const updateCount = addons.filter((a2) => a2.updateAvailable && !a2.isIgnored).length;
      return { ...state, installedAddons: addons, updateCount };
    }
    case "SET_SCANNING":
...
function AppProvider({ children }) {
  ...
  const patchSettings = reactExports.useCallback(async (patch) => {
    const updated = await api.patchSettings(patch);
    dispatch({ type: "SET_SETTINGS", settings: updated });
  }, [api]);
  const switchInstallation = reactExports.useCallback(async (id2) => {
    dispatch({ type: "SET_ACTIVE_INSTALLATION", id: id2 });
    await api.patchSettings({ activeInstallationId: id2 });
    const addons = await api.getInstalledAddons(id2);
    dispatch({ type: "SET_INSTALLED_ADDONS", addons });
  }, [api]);
  const scanAddons = reactExports.useCallback(async () => {
    const id2 = state.activeInstallationId;
    if (!id2) return;
    dispatch({ type: "SET_SCANNING", value: true });
    try {
      const addons = await api.scanAddons(id2);
      dispatch({ type: "SET_INSTALLED_ADDONS", addons });
      zt.success(`Found ${addons.length} addons`);
    } catch (err) {
      zt.error(`Scan failed: ${err.message}`);
    } finally {
      dispatch({ type: "SET_SCANNING", value: false });
    }
  }, [api, state.activeInstallationId]);
  const checkUpdates = reactExports.useCallback(async () => {
    const id2 = state.activeInstallationId;
    if (!id2) return;
    dispatch({ type: "SET_CHECKING_UPDATES", value: true });
    try {
      const addons = await api.checkUpdates(id2);
      dispatch({ type: "SET_INSTALLED_ADDONS", addons });
      const count = addons.filter((a2) => a2.updateAvailable && !a2.isIgnored).length;
      if (count > 0) zt.success(`${count} update${count > 1 ? "s" : ""} available`);
      else zt.success("All addons are up to date");
    } catch (err) {
      zt.error(`Update check failed: ${err.message}`);
    } finally {
  ...
  const updateAddon = reactExports.useCallback(async (addonId) => {
    const id2 = state.activeInstallationId;
    if (!id2) return;
    const t2 = zt.loading("Updating…");
    try {
      const updated = await api.updateAddon({ addonId, installationId: id2 });
      dispatch({ type: "UPSERT_ADDON", addon: updated });
      zt.success(`${updated.name} updated`, { id: t2 });
    } catch (err) {
      zt.error(`Update failed: ${err.message}`, { id: t2 });
    }
  }, [api, state.activeInstallationId]);
  const updateAllAddons = reactExports.useCallback(async () => {
    const id2 = state.activeInstallationId;
    if (!id2) return;
    const t2 = zt.loading("Updating all addons…");
    try {
      const updated = await api.updateAllAddons(id2);
      for (const addon of updated) dispatch({ type: "UPSERT_ADDON", addon });
      zt.success(`Updated ${updated.length} addon${updated.length !== 1 ? "s" : ""}`, { id: t2 });
    } catch (err) {
      zt.error(`Update failed: ${err.message}`, { id: t2 });
    }
  }, [api, state.activeInstallationId]);
  const uninstallAddon = reactExports.useCallback(async (addonId) => {
    const id2 = state.activeInstallationId;
    if (!id2) return;
    const addon = state.installedAddons.find((a2) => a2.id === addonId);
    const t2 = zt.loading(`Removing ${addon?.name ?? addonId}…`);
    try {
      await api.uninstallAddon({ addonId, installationId: id2 });
      dispatch({ type: "REMOVE_ADDON", addonId });
      zt.success("Addon removed", { id: t2 });
    } catch (err) {
      zt.error(`Uninstall failed: ${err.message}`, { id: t2 });
    }
  }, [api, state.activeInstallationId, state.installedAddons]);
  const linkAddonToProvider = reactExports.useCallback(async (addonId, result) => {
    const id2 = state.activeInstallationId;
    if (!id2) return;
    const t2 = zt.loading("Linking addon…");
    try {
      const updated = await api.linkAddonToProvider({ addonId, installationId: id2, result });
      dispatch({ type: "REMOVE_ADDON", addonId });
      dispatch({ type: "UPSERT_ADDON", addon: updated });
      zt.success(`Linked to ${result.name}`, { id: t2 });
    } catch (err) {
      zt.error(`Link failed: ${err.message}`, { id: t2 });
    }
  }, [api, state.activeInstallationId]);
  const addInstallation = reactExports.useCallback(async (installation) => {
    const current = state.settings?.wowInstallations ?? [];
    if (current.some((i2) => i2.addonsPath === installation.addonsPath)) return;
  ...
  const removeInstallation = reactExports.useCallback(async (id2) => {
    const current = state.settings?.wowInstallations ?? [];
    const updated = current.filter((i2) => i2.id !== id2);
    const activeId = state.activeInstallationId === id2 ? updated[0]?.id ?? null : state.activeInstallationId;
    await patchSettings({ wowInstallations: updated, activeInstallationId: activeId });
    if (activeId && activeId !== id2) {
  ...
  reactExports.useEffect(() => {
    ...
    const unWow = api.onWowDetected(async (installations) => {
      ...
      zt.success(`Detected ${installations.length} WoW installation(s)`);
    });
    const unCheck = api.onTriggerUpdateCheck(async (id2) => {
      if (id2 === state.activeInstallationId) await checkUpdates();
    });
    const unBg = api.onBackgroundUpdated(async (id2) => {
      if (id2 === state.activeInstallationId) {
        const addons = await api.getInstalledAddons(id2);
        dispatch({ type: "SET_INSTALLED_ADDONS", addons });
      }
    });
...
function Titlebar() {
  return /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "drag-region flex items-center h-9 bg-wow-dark border-b border-gray-800 px-4 shrink-0", children: /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "no-drag flex items-center gap-2 select-none pointer-events-none", children: /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "text-wow-gold text-sm font-bold tracking-wide", children: "⚔ WoW Warden" }) }) });
}
function Sidebar() {
  const { updateCount, installations, activeInstallationId, switchInstallation, isScanning, isCheckingUpdates } = useApp();
  const navItems = [
    { to: "/", icon: "📦", label: "My Addons", badge: updateCount || void 0 },
    ...
    /* @__PURE__ */ jsxRuntimeExports.jsx("nav", { className: "flex-1 p-3 space-y-1", children: navItems.map((item) => /* @__PURE__ */ jsxRuntimeExports.jsxs(
      NavLink,
      {
        ...
        className: ({ isActive }) => `flex items-center gap-3 px-3 py-2 rounded-lg text-sm font-medium transition-colors
               ${isActive ? "bg-wow-gold/20 text-wow-gold" : "text-gray-400 hover:text-gray-100 hover:bg-gray-700/50"}`,
        children: [
          ...
          item.badge ? /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "badge bg-amber-500 text-gray-900 font-bold", children: item.badge }) : null
        ]
      },
      item.to
    )) }),
    installations.length > 0 && /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "p-3 border-t border-gray-800", children: [
      ...
      /* @__PURE__ */ jsxRuntimeExports.jsx(
        "select",
        {
          ...
          onChange: (e2) => switchInstallation(e2.target.value),
          className: "input w-full text-xs",
          children: installations.map((inst) => /* @__PURE__ */ jsxRuntimeExports.jsx("option", { value: inst.id, children: inst.displayName }, inst.id))
        }
      ),
...
function normalizeVersion(v2) {
  if (!v2) return "";
  let s2 = v2.trim();
  const uIdx = s2.lastIndexOf("_");
  if (uIdx >= 0) s2 = s2.slice(uIdx + 1);
  s2 = s2.replace(/^[^\d.]+/, "");
  return s2 || v2;
}
const PROVIDER_LABEL$1 = {
  wago: "Wago",
  curseforge: "CurseForge",
  wowinterface: "WoWInterface",
  github: "GitHub"
};
const PROVIDER_COLORS$1 = {
  wago: "badge-wago",
  curseforge: "badge-curseforge",
  wowinterface: "badge-wowinterface",
  github: "badge-github"
};
function fmtNumber$1(n2) {
  if (n2 >= 1e6) return `${(n2 / 1e6).toFixed(1)}M`;
  if (n2 >= 1e3) return `${(n2 / 1e3).toFixed(0)}K`;
  return String(n2);
}
function LinkAddonDialog({ addon, onClose }) {
  const { linkAddonToProvider, settings } = useApp();
  const [query, setQuery] = reactExports.useState(addon.name);
  ...
  return (
    ...
    /* @__PURE__ */ jsxRuntimeExports.jsx(
      "div",
      {
        ...
        children: /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "card w-full max-w-lg mx-4 p-5 flex flex-col gap-4 max-h-[80vh]", children: [
          /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex items-start justify-between gap-3", children: [
            /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { children: [
              /* @__PURE__ */ jsxRuntimeExports.jsx("h2", { className: "font-semibold text-gray-100 text-sm", children: addon.provider === "unknown" ? "Link to Provider" : "Change Provider" }),
              /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "text-gray-500 text-xs mt-0.5", children: addon.provider === "unknown" ? /* @__PURE__ */ jsxRuntimeExports.jsxs(jsxRuntimeExports.Fragment, { children: [
                "Search for ",
                /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "text-gray-300", children: addon.name }),
                " and select the matching result to enable update tracking."
              ] }) : /* @__PURE__ */ jsxRuntimeExports.jsxs(jsxRuntimeExports.Fragment, { children: [
                "Search for ",
                /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "text-gray-300", children: addon.name }),
                " and select a result to switch which provider is used for updates."
              ] }) })
            ] }),
            /* @__PURE__ */ jsxRuntimeExports.jsx("button", { className: "btn-ghost text-xs py-1 px-2 shrink-0", onClick: onClose, children: "✕" })
          ] }),
          ...
          /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "overflow-y-auto flex flex-col gap-2 min-h-0", children: [
            loading && /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "text-gray-500 text-xs text-center py-4", children: "Searching…" }),
            !loading && results.length === 0 && query.trim() && /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "text-gray-500 text-xs text-center py-4", children: "No results found." }),
            results.map((result) => /* @__PURE__ */ jsxRuntimeExports.jsxs(
              "div",
              {
                className: "card px-3 py-2 flex items-center gap-3 hover:border-gray-600 transition-colors",
                children: [
                  /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "w-9 h-9 rounded bg-gray-800 shrink-0 overflow-hidden flex items-center justify-center text-lg", children: result.thumbnailUrl ? /* @__PURE__ */ jsxRuntimeExports.jsx("img", { src: result.thumbnailUrl, alt: "", className: "w-full h-full object-cover", onError: (e2) => {
                    e2.target.style.display = "none";
                  } }) : "🧩" }),
                  /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex-1 min-w-0", children: [
                    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex items-center gap-1.5 flex-wrap", children: [
                      /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "text-gray-100 text-xs font-medium truncate", children: result.name }),
                      /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: PROVIDER_COLORS$1[result.provider] ?? "badge-unknown", children: PROVIDER_LABEL$1[result.provider] ?? result.provider })
                    ] }),
                    /* @__PURE__ */ jsxRuntimeExports.jsxs("p", { className: "text-gray-500 text-xs", children: [
                      "by ",
                      result.author,
                      result.downloadCount > 0 && /* @__PURE__ */ jsxRuntimeExports.jsxs(jsxRuntimeExports.Fragment, { children: [
                        " · ",
                        fmtNumber$1(result.downloadCount),
                        " dl"
                      ] }),
                      result.latestVersion && /* @__PURE__ */ jsxRuntimeExports.jsxs(jsxRuntimeExports.Fragment, { children: [
                        " · v",
...
function VersionPickerDialog({ addon, onClose, onPinned }) {
  ...
  const loadVersions = async () => {
    ...
    setLoading(true);
    try {
      const v2 = await window.api.getAddonVersions({
        addonId: addon.id,
        installationId: activeInstallationId
      });
      setVersions(v2);
    } catch (err) {
      zt.error(`Failed to load versions: ${err.message}`);
    } finally {
      setLoading(false);
    }
  };
  const handlePinVersion = async (ver) => {
    if (!activeInstallationId) return;
    setInstalling(ver.version);
    const t2 = zt.loading(`Installing ${addon.name} v${normalizeVersion(ver.version)}…`);
    try {
      ...
      zt.success(`Pinned to v${normalizeVersion(ver.version)}`, { id: t2 });
      onPinned(updated);
      onClose();
    } catch (err) {
      zt.error(err.message ?? "Install failed", { id: t2 });
    } finally {
      setInstalling(null);
    }
  };
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsx(
    "div",
    {
      ...
      children: /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "card w-full max-w-lg mx-4 p-5 flex flex-col gap-4 max-h-[80vh]", children: [
        /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex items-start justify-between gap-3", children: [
          /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { children: [
            /* @__PURE__ */ jsxRuntimeExports.jsx("h2", { className: "font-semibold text-gray-100 text-sm", children: "Choose Version" }),
            /* @__PURE__ */ jsxRuntimeExports.jsxs("p", { className: "text-gray-500 text-xs mt-0.5", children: [
              "Select a specific version of ",
              /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "text-gray-300", children: addon.name }),
              " to install.",
              " ",
        ...
        /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "text-xs text-gray-500 bg-gray-800/50 rounded-lg px-3 py-2", children: [
          "Currently installed: ",
          /* @__PURE__ */ jsxRuntimeExports.jsxs("span", { className: "text-gray-300", children: [
            "v",
            addon.version
          ] }),
          pinnedNorm && /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "text-amber-400 ml-2", children: "(pinned)" })
        ] }),
        /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "overflow-y-auto flex flex-col gap-1.5 min-h-0", children: [
          loading && /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "text-gray-500 text-xs text-center py-8", children: "Loading versions…" }),
          !loading && versions.length === 0 && /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "text-gray-500 text-xs text-center py-8", children: "No versions available from this provider." }),
          versions.map((ver) => {
                ...
                children: [
                  /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex-1 min-w-0", children: [
                    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex items-center gap-2 flex-wrap", children: [
                      /* @__PURE__ */ jsxRuntimeExports.jsxs("span", { className: "text-gray-100 text-xs font-medium truncate", children: [
                        "v",
                        normalizeVersion(ver.version)
                      ] }),
                      ver.releaseType !== "stable" && /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: `text-xs ${CHANNEL_COLORS[ver.releaseType]}`, children: ver.releaseType }),
                      isCurrent && /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "badge bg-wow-gold/20 text-wow-gold text-xs", children: "Current" }),
                      isPinned && /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "badge bg-amber-600/20 text-amber-400 text-xs", children: "Pinned" })
                    ] }),
                    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex items-center gap-3 text-xs text-gray-500 mt-0.5", children: [
                      ver.displayName && ver.displayName !== ver.version && /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "truncate", children: ver.displayName }),
                      ver.releaseDate && /* @__PURE__ */ jsxRuntimeExports.jsx("span", { children: formatDate$1(ver.releaseDate) }),
                      ver.gameVersions && ver.gameVersions.length > 0 && /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "truncate", children: ver.gameVersions.slice(0, 3).join(", ") })
                    ] })
                  ] }),
                  /* @__PURE__ */ jsxRuntimeExports.jsx(
                    "button",
                    {
                      className: `text-xs py-1 px-3 shrink-0 ${isPinned ? "btn-secondary" : "btn-primary"}`,
                      onClick: () => handlePinVersion(ver),
                      disabled: installing !== null || isPinned === true,
                      children: installing === ver.version ? "Installing…" : isPinned ? "Pinned" : "Install & Pin"
                    }
                  )
                ]
              },
...
function AddonRow({ addon: initialAddon }) {
  const { updateAddon, uninstallAddon, installations, activeInstallationId } = useApp();
  const [addon, setAddon] = reactExports.useState(initialAddon);
  ...
  const [versionDialogOpen, setVersionDialogOpen] = reactExports.useState(false);
  React.useEffect(() => {
    setAddon(initialAddon);
  }, [initialAddon]);
  const handleUpdate = async (e2) => {
    e2.stopPropagation();
    await updateAddon(addon.id);
  };
  const handleUninstall = async (e2) => {
    e2.stopPropagation();
    if (!confirming) {
      setConfirming(true);
      return;
  ...
  const openWebsite = (e2) => {
    e2.stopPropagation();
    if (addon.websiteUrl) window.api.openUrl(addon.websiteUrl);
  };
  const openFolder = (e2) => {
    e2.stopPropagation();
    const installation = installations.find((i2) => i2.id === activeInstallationId);
    ...
    "div",
    {
      ...
      children: [
        /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex items-center gap-3", children: [
          /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "w-10 h-10 rounded-lg bg-gray-800 shrink-0 overflow-hidden flex items-center justify-center text-xl", children: addon.thumbnailUrl ? /* @__PURE__ */ jsxRuntimeExports.jsx("img", { src: addon.thumbnailUrl, alt: "", className: "w-full h-full object-cover" }) : "🧩" }),
          /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex-1 min-w-0", children: [
            /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex items-center gap-2 flex-wrap", children: [
              /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "font-semibold text-sm text-gray-100 truncate", children: addon.name }),
              addon.pinnedVersion && /* @__PURE__ */ jsxRuntimeExports.jsxs("span", { className: "badge bg-amber-700 text-amber-200", children: [
                "Pinned v",
                normalizeVersion(addon.pinnedVersion)
              ] }),
              addon.updateAvailable && !addon.isIgnored && /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "badge-update", children: "Update" }),
              addon.isIgnored && /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "badge-ignored", children: "Ignored" }),
              /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: `badge-${addon.provider}`, children: PROVIDER_LABEL[addon.provider] })
            ] }),
            /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex items-center gap-3 mt-0.5 text-xs text-gray-500", children: [
              /* @__PURE__ */ jsxRuntimeExports.jsxs("span", { children: [
                "v",
                addon.version
              ] }),
              addon.author && /* @__PURE__ */ jsxRuntimeExports.jsxs("span", { children: [
                "by ",
                addon.author
              ] }),
              addon.updateAvailable && addon.latestVersion && /* @__PURE__ */ jsxRuntimeExports.jsxs("span", { className: "text-amber-400", children: [
                "→ v",
                normalizeVersion(addon.latestVersion)
              ] })
            ] })
          ] }),
          /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex items-center gap-2 no-drag shrink-0", onClick: (e2) => e2.stopPropagation(), children: [
            addon.updateAvailable && !addon.isIgnored && !addon.pinnedVersion && /* @__PURE__ */ jsxRuntimeExports.jsx("button", { className: "btn-success text-xs py-1 px-3", onClick: handleUpdate, children: "Update" }),
            addon.updateAvailable && addon.pinnedVersion && /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "text-amber-400 text-xs", title: "Update available but version is pinned", children: "New version available" }),
            addon.websiteUrl && /* @__PURE__ */ jsxRuntimeExports.jsx("button", { className: "btn-ghost text-xs py-1 px-2", onClick: openWebsite, title: "Open website", children: "↗" }),
            /* @__PURE__ */ jsxRuntimeExports.jsx("button", { className: "btn-ghost text-xs py-1 px-2", onClick: openFolder, title: "Open addon folder in Explorer", children: "📁" })
          ] }),
          /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: `text-gray-600 text-xs transition-transform ${expanded ? "rotate-180" : ""}`, children: "▼" })
        ] }),
        expanded && /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "mt-3 pt-3 border-t border-gray-800 space-y-3", children: [
          addon.notes && /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "text-gray-400 text-xs leading-relaxed", children: addon.notes }),
          /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "grid grid-cols-2 gap-2 text-xs", children: [
            /* @__PURE__ */ jsxRuntimeExports.jsx(Detail, { label: "Version", value: `v${addon.version}` }),
            addon.latestVersion && normalizeVersion(addon.latestVersion) !== normalizeVersion(addon.version) && /* @__PURE__ */ jsxRuntimeExports.jsx(Detail, { label: "Latest", value: `v${normalizeVersion(addon.latestVersion)}`, className: "text-amber-400" }),
            /* @__PURE__ */ jsxRuntimeExports.jsx(Detail, { label: "Source", value: PROVIDER_LABEL[addon.provider] }),
            addon.gameVersion && /* @__PURE__ */ jsxRuntimeExports.jsx(Detail, { label: "Interface", value: addon.gameVersion }),
            /* @__PURE__ */ jsxRuntimeExports.jsx(Detail, { label: "Installed", value: formatDate(addon.installedAt) }),
            /* @__PURE__ */ jsxRuntimeExports.jsx(Detail, { label: "Updated", value: timeAgo(addon.updatedAt) }),
            /* @__PURE__ */ jsxRuntimeExports.jsx(Detail, { label: "Directories", value: addon.directories.join(", ") })
          ] }),
          addon.pinnedVersion && addon.updateAvailable && addon.latestVersion && /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "text-xs bg-amber-400/10 border border-amber-400/20 rounded px-3 py-1.5 text-amber-300", children: [
            "Version pinned to v",
            normalizeVersion(addon.pinnedVersion),
            ".",
            " ",
          ...
          /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex gap-2 flex-wrap", children: [
            addon.provider !== "unknown" && /* @__PURE__ */ jsxRuntimeExports.jsx(
              "button",
              {
            ...
            /* @__PURE__ */ jsxRuntimeExports.jsx(
              "button",
              {
                ...
                title: addon.provider === "unknown" ? "Link this addon to a provider to enable update tracking" : "Change which provider this addon is linked to",
                children: addon.provider === "unknown" ? "⇆ Link to Provider" : "⇆ Change Provider"
              }
            ),
            /* @__PURE__ */ jsxRuntimeExports.jsx(
              "button",
              {
                ...
                children: confirming ? "Confirm Remove" : "Uninstall"
              }
            ),
            confirming && /* @__PURE__ */ jsxRuntimeExports.jsx("button", { className: "btn-ghost text-xs py-1 px-3", onClick: (e2) => {
              e2.stopPropagation();
              setConfirming(false);
            }, children: "Cancel" })
          ] })
        ] }),
        linkDialogOpen && /* @__PURE__ */ jsxRuntimeExports.jsx(
          LinkAddonDialog,
          {
            addon,
        ...
        versionDialogOpen && /* @__PURE__ */ jsxRuntimeExports.jsx(
          VersionPickerDialog,
          {
            addon,
...
function Detail({ label, value, className = "text-gray-300" }) {
  return /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { children: [
    /* @__PURE__ */ jsxRuntimeExports.jsxs("span", { className: "text-gray-600", children: [
      label,
      ": "
    ] }),
    /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className, children: value })
  ] });
}
function AutoUpdateToggle({ addon }) {
  const { activeInstallationId } = useApp();
  const toggle = async (e2) => {
    e2.stopPropagation();
    if (!activeInstallationId) return;
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsx(
    "button",
    {
      ...
      onClick: toggle,
      children: addon.autoUpdate ? "✓ Auto-update" : "○ Auto-update"
    }
  );
}
function IgnoreToggle({ addon }) {
  const { activeInstallationId } = useApp();
  const toggle = async (e2) => {
    e2.stopPropagation();
    if (!activeInstallationId) return;
...
function UnpinButton({ addon, onUnpinned }) {
  const { activeInstallationId } = useApp();
  const handleUnpin = async (e2) => {
    e2.stopPropagation();
    if (!activeInstallationId) return;
    try {
      ...
      onUnpinned(updated);
      zt.success(`${addon.name} unpinned — normal updates resumed`);
    } catch (err) {
      zt.error(`Unpin failed: ${err.message}`);
    }
  };
  return /* @__PURE__ */ jsxRuntimeExports.jsx(
    "button",
...
function LoadingSpinner({ size = "md", message }) {
  return /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex flex-col items-center gap-3", children: [
    /* @__PURE__ */ jsxRuntimeExports.jsx(
      "div",
      {
        className: `${sizes[size]} border-2 border-gray-700 border-t-wow-gold rounded-full animate-spin`
      }
    ),
    message && /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "text-gray-500 text-sm", children: message })
  ] });
}
function EmptyState({ icon = "📭", title, description, action }) {
  return /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex flex-col items-center justify-center gap-4 py-20 text-center", children: [
    /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "text-6xl opacity-40", children: icon }),
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx("h3", { className: "text-gray-300 font-semibold", children: title }),
      description && /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "text-gray-600 text-sm mt-1 max-w-xs", children: description })
    ] }),
    action
  ] });
}
function MyAddons() {
  ...
  const filtered = reactExports.useMemo(() => {
    ...
    if (q2) list = list.filter(
      (a2) => a2.name.toLowerCase().includes(q2) || a2.author.toLowerCase().includes(q2)
    );
    if (filter === "updates") list = list.filter((a2) => a2.updateAvailable && !a2.isIgnored);
    if (filter === "ignored") list = list.filter((a2) => a2.isIgnored);
    if (sort === "name") list.sort((a2, b) => a2.name.localeCompare(b.name));
    if (sort === "author") list.sort((a2, b) => a2.author.localeCompare(b.author));
    if (sort === "status") list.sort(
      (a2, b) => Number(b.updateAvailable) - Number(a2.updateAvailable) || a2.name.localeCompare(b.name)
    );
    return list;
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex flex-col h-full page-enter", children: [
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex items-center gap-3 px-5 py-3 border-b border-gray-800 shrink-0", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx("h2", { className: "text-sm font-semibold text-gray-200 w-28 shrink-0", children: "My Addons" }),
      /* @__PURE__ */ jsxRuntimeExports.jsx(
        "input",
        {
      ...
      /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "flex bg-gray-800 rounded-lg p-0.5 gap-0.5", children: ["all", "updates", "ignored"].map((f2) => /* @__PURE__ */ jsxRuntimeExports.jsxs(
        "button",
        {
          onClick: () => setFilter(f2),
          className: `px-3 py-1 rounded-md text-xs font-medium transition-colors capitalize
                ${filter === f2 ? "bg-wow-gold text-gray-900" : "text-gray-400 hover:text-gray-200"}`,
          children: [
            f2,
            f2 === "updates" && updateCount > 0 ? ` (${updateCount})` : ""
          ]
        },
        f2
      )) }),
      /* @__PURE__ */ jsxRuntimeExports.jsxs(
        "select",
        {
          ...
          value: sort,
          onChange: (e2) => setSort(e2.target.value),
          children: [
            /* @__PURE__ */ jsxRuntimeExports.jsx("option", { value: "name", children: "Sort: Name" }),
            /* @__PURE__ */ jsxRuntimeExports.jsx("option", { value: "status", children: "Sort: Status" }),
            /* @__PURE__ */ jsxRuntimeExports.jsx("option", { value: "author", children: "Sort: Author" })
          ]
        }
      ),
      /* @__PURE__ */ jsxRuntimeExports.jsx(
      ...
      /* @__PURE__ */ jsxRuntimeExports.jsx(
        "button",
        {
          ...
          children: isCheckingUpdates ? "⟳ Checking…" : "↻ Check Updates"
        }
      ),
      updateCount > 0 && /* @__PURE__ */ jsxRuntimeExports.jsxs("button", { className: "btn-primary text-xs py-1.5 px-3", onClick: updateAllAddons, children: [
        "Update All (",
        updateCount,
        ")"
      ] })
    ] }),
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex items-center gap-4 px-5 py-2 border-b border-gray-800/50 shrink-0 text-xs text-gray-500", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsxs("span", { children: [
      ...
      updateCount > 0 && /* @__PURE__ */ jsxRuntimeExports.jsxs("span", { className: "text-amber-400", children: [
        updateCount,
        " update",
        updateCount !== 1 ? "s" : "",
        " available"
      ] }),
      filtered.length !== installedAddons.length && /* @__PURE__ */ jsxRuntimeExports.jsxs("span", { children: [
        "Showing ",
        filtered.length
      ] })
    ] }),
    /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "flex-1 overflow-y-auto px-5 py-3 space-y-2", children: isScanning && installedAddons.length === 0 ? /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "flex justify-center pt-20", children: /* @__PURE__ */ jsxRuntimeExports.jsx(LoadingSpinner, { message: "Scanning AddOns directory…" }) }) : filtered.length === 0 ? installedAddons.length === 0 ? /* @__PURE__ */ jsxRuntimeExports.jsx(
      EmptyState,
      {
        icon: "📭",
        title: "No addons found",
        description: "Click Scan to detect addons from your WoW AddOns folder.",
        action: /* @__PURE__ */ jsxRuntimeExports.jsx("button", { className: "btn-primary", onClick: scanAddons, disabled: isScanning, children: "⟳ Scan Addons" })
      }
    ) : /* @__PURE__ */ jsxRuntimeExports.jsx(
      EmptyState,
      {
    ...
    ) : filtered.map((addon) => /* @__PURE__ */ jsxRuntimeExports.jsx(AddonRow, { addon }, addon.id)) })
  ] });
}
...
function SearchResultCard({ result, installedAddons }) {
  ...
  const handleInstall = async () => {
    if (!activeInstallationId || !result.downloadUrl) {
      zt.error("No download URL available for this addon.");
      return;
    }
    setInstalling(true);
    ...
    try {
      ...
      zt.success(`${result.name} installed!`, { id: t2 });
    } catch (err) {
      zt.error(err.message ?? "Install failed", { id: t2 });
    } finally {
      setInstalling(false);
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "card p-4 flex gap-4 hover:border-gray-700 transition-colors", children: [
    /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "w-16 h-16 rounded-lg bg-gray-800 shrink-0 overflow-hidden flex items-center justify-center text-3xl", children: result.thumbnailUrl ? /* @__PURE__ */ jsxRuntimeExports.jsx("img", { src: result.thumbnailUrl, alt: "", className: "w-full h-full object-cover", onError: (e2) => {
      e2.target.style.display = "none";
    } }) : "🧩" }),
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex-1 min-w-0", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex items-start justify-between gap-3", children: [
        /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "min-w-0", children: [
          /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex items-center gap-2 flex-wrap", children: [
            /* @__PURE__ */ jsxRuntimeExports.jsx("h3", { className: "font-semibold text-gray-100 text-sm truncate", children: result.name }),
            /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: PROVIDER_COLORS[result.provider] ?? "badge-unknown", children: result.provider }),
            installed && /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "badge-installed", children: "Installed" })
          ] }),
          /* @__PURE__ */ jsxRuntimeExports.jsxs("p", { className: "text-gray-500 text-xs mt-0.5", children: [
            "by ",
            result.author,
            result.downloadCount > 0 && /* @__PURE__ */ jsxRuntimeExports.jsxs(jsxRuntimeExports.Fragment, { children: [
              " · ",
              fmtNumber(result.downloadCount),
              " downloads"
            ] }),
            result.latestVersion && /* @__PURE__ */ jsxRuntimeExports.jsxs(jsxRuntimeExports.Fragment, { children: [
              " · v",
        ...
        /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex items-center gap-2 shrink-0", children: [
          result.websiteUrl && /* @__PURE__ */ jsxRuntimeExports.jsx("button", { className: "btn-ghost text-xs py-1 px-2", onClick: openWebsite, title: "Open website", children: "↗" }),
          installed ? /* @__PURE__ */ jsxRuntimeExports.jsx("button", { className: "btn-secondary text-xs py-1 px-3", disabled: true, children: "Installed" }) : /* @__PURE__ */ jsxRuntimeExports.jsx(
            "button",
            {
              className: "btn-primary text-xs py-1 px-3",
              onClick: handleInstall,
              disabled: installing || !result.downloadUrl,
              title: !result.downloadUrl ? "No download URL available" : void 0,
              children: installing ? "Installing…" : "Install"
            }
          )
        ] })
      ] }),
      result.summary && /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "text-gray-400 text-xs mt-2 line-clamp-2", children: result.summary }),
      result.categories && result.categories.length > 0 && /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "flex gap-1 mt-2 flex-wrap", children: result.categories.slice(0, 4).map((cat) => /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "badge bg-gray-800 text-gray-400 text-xs", children: cat }, cat)) })
    ] })
  ] });
}
const PROVIDERS = [
...
function Browse() {
  ...
  const [ghQuery, setGhQuery] = reactExports.useState("");
  ...
  const [mode, setMode] = reactExports.useState("home");
  const activeInstall = installations.find((i2) => i2.id === activeInstallationId);
  const defaultFlavor = activeInstall?.flavor ?? "retail";
  const [flavor, setFlavor] = reactExports.useState(defaultFlavor);
  const PAGE_SIZE = 20;
  const inputRef = reactExports.useRef(null);
  reactExports.useEffect(() => {
    loadCategories();
  ...
  const doSearch = reactExports.useCallback(async (newQuery, newPage, append = false, categoryId, sort) => {
    setLoading(true);
    try {
      const res = await window.api.searchAddons({
        query: newQuery,
        provider: categoryId ? "curseforge" : provider === "all" ? void 0 : provider,
        flavor,
        page: newPage,
        pageSize: PAGE_SIZE,
        categoryId,
      ...
      zt.error(`Search failed: ${err.message}`);
    } finally {
      setLoading(false);
    }
  ...
  const handleSortChange = (newSort) => {
    ...
    if (mode === "category" && selectedCategory) {
      ...
      setResults([]);
      setPage(1);
      doSearch(query, 1, false, void 0, newSort);
    }
  };
  ...
  const handleGitHubLookup = async (e2) => {
    e2.preventDefault();
    if (!ghQuery.trim()) return;
    const t2 = zt.loading("Looking up GitHub repo…");
    try {
      const ownerRepo = ghQuery.replace(/^https?:\/\/github\.com\//, "").replace(/\.git$/, "").trim();
      const result = await window.api.githubLookup(ownerRepo);
      if (result) {
        setMode("search");
        setResults([result, ...results.filter((r2) => r2.externalId !== result.externalId)]);
        zt.success("Found GitHub release!", { id: t2 });
      } else {
        zt.error("No releases found for that repo.", { id: t2 });
      }
    } catch (err) {
      zt.error(err.message ?? "Lookup failed", { id: t2 });
    }
  };
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex flex-col h-full page-enter", children: [
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "px-5 py-3 border-b border-gray-800 shrink-0 space-y-3", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex items-center justify-between", children: [
        /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex items-center gap-3", children: [
          mode !== "home" && /* @__PURE__ */ jsxRuntimeExports.jsx(
            "button",
            {
          ...
          /* @__PURE__ */ jsxRuntimeExports.jsx("h2", { className: "text-sm font-semibold text-gray-200", children: mode === "category" && selectedCategory ? selectedCategory.name : "Browse Addons" })
        ] }),
        (mode === "category" || mode === "search") && /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex items-center gap-2", children: [
          /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "text-xs text-gray-500", children: "Sort:" }),
          /* @__PURE__ */ jsxRuntimeExports.jsx(
            "select",
            {
              className: "input text-xs py-1",
              value: sortBy,
              onChange: (e2) => handleSortChange(e2.target.value),
              children: SORT_OPTIONS.map((s2) => /* @__PURE__ */ jsxRuntimeExports.jsx("option", { value: s2.value, children: s2.label }, s2.value))
            }
          )
        ] })
      ] }),
      /* @__PURE__ */ jsxRuntimeExports.jsxs("form", { onSubmit: handleSearch, className: "flex gap-2", children: [
        ...
        /* @__PURE__ */ jsxRuntimeExports.jsx(
          "select",
          {
            ...
            children: PROVIDERS.map((p2) => /* @__PURE__ */ jsxRuntimeExports.jsx("option", { value: p2.value, children: p2.label }, p2.value))
          }
        ),
        /* @__PURE__ */ jsxRuntimeExports.jsx(
          "select",
          {
            className: "input text-xs",
            value: flavor,
            onChange: (e2) => setFlavor(e2.target.value),
            children: Object.entries(FLAVOR_LABELS).map(([k2, v2]) => /* @__PURE__ */ jsxRuntimeExports.jsx("option", { value: k2, children: v2 }, k2))
          }
        ),
        /* @__PURE__ */ jsxRuntimeExports.jsx("button", { type: "submit", className: "btn-primary px-5", disabled: loading, children: loading && mode === "search" ? "⟳" : "Search" })
      ] }),
      provider === "curseforge" && !settings?.curseForgApiKey && /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "text-xs text-amber-400 bg-amber-400/10 border border-amber-400/30 rounded px-3 py-1.5", children: [
        "CurseForge requires an API key. Add yours in",
        " ",
        /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "font-semibold", children: "Settings → API Keys" }),
        "."
      ] }),
      /* @__PURE__ */ jsxRuntimeExports.jsxs("form", { onSubmit: handleGitHubLookup, className: "flex gap-2", children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx(
          "input",
          {
            ...
            value: ghQuery,
            onChange: (e2) => setGhQuery(e2.target.value)
          }
        ),
        /* @__PURE__ */ jsxRuntimeExports.jsx("button", { type: "submit", className: "btn-secondary text-xs px-3", children: "GitHub Lookup" })
      ] })
    ] }),
    /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "flex-1 overflow-y-auto px-5 py-3 space-y-2", children: mode === "home" ? (
      /* ── Category Grid ─────────────────────────────────────────── */
      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { children: [
        !settings?.curseForgApiKey && /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "text-xs text-amber-400 bg-amber-400/10 border border-amber-400/30 rounded px-3 py-2 mb-4", children: [
          "Category browsing requires a CurseForge API key. Add yours in",
          " ",
          /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "font-semibold", children: "Settings → API Keys" }),
          " to browse addons by category."
        ] }),
        /* @__PURE__ */ jsxRuntimeExports.jsx("h3", { className: "section-header mb-3", children: "Browse by Category" }),
        categoriesLoading ? /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "flex justify-center pt-10", children: /* @__PURE__ */ jsxRuntimeExports.jsx(LoadingSpinner, { message: "Loading categories…" }) }) : categories.length === 0 ? /* @__PURE__ */ jsxRuntimeExports.jsx(
          EmptyState,
          {
            icon: "📁",
            title: "No categories available",
            description: settings?.curseForgApiKey ? "Could not load categories. Try again later." : "Add a CurseForge API key in Settings to browse addons by category."
          }
        ) : /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-2", children: categories.map((cat) => /* @__PURE__ */ jsxRuntimeExports.jsx(
          "button",
          {
            ...
            children: /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex items-center gap-2", children: [
              cat.iconUrl ? /* @__PURE__ */ jsxRuntimeExports.jsx(
                "img",
                {
                  src: cat.iconUrl,
                  alt: "",
              ...
              ) : /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "text-lg", children: getCategoryIcon(cat.name) }),
              /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "text-sm text-gray-200 group-hover:text-wow-gold transition-colors truncate", children: cat.name })
            ] })
          },
          cat.id
        )) })
      ...
      /* @__PURE__ */ jsxRuntimeExports.jsx(jsxRuntimeExports.Fragment, { children: loading && results.length === 0 ? /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "flex justify-center pt-20", children: /* @__PURE__ */ jsxRuntimeExports.jsx(LoadingSpinner, { message: mode === "category" ? `Loading ${selectedCategory?.name ?? "category"}…` : "Searching…" }) }) : results.length === 0 ? /* @__PURE__ */ jsxRuntimeExports.jsx(
        EmptyState,
        {
          icon: mode === "category" ? "📁" : "🔍",
          title: mode === "category" ? "No addons found" : "Search for addons",
          description: mode === "category" ? `No addons found in ${selectedCategory?.name ?? "this category"}.` : "Enter an addon name above to search across all configured sources."
        }
      ) : /* @__PURE__ */ jsxRuntimeExports.jsxs(jsxRuntimeExports.Fragment, { children: [
        /* @__PURE__ */ jsxRuntimeExports.jsxs("p", { className: "text-xs text-gray-600 pb-1", children: [
          results.length,
          " result",
          results.length !== 1 ? "s" : "",
          mode === "category" && selectedCategory && /* @__PURE__ */ jsxRuntimeExports.jsxs(jsxRuntimeExports.Fragment, { children: [
            " in ",
            /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "text-gray-400", children: selectedCategory.name })
          ] })
        ] }),
        results.map((r2) => /* @__PURE__ */ jsxRuntimeExports.jsx(
          ...
          `${r2.provider}:${r2.externalId}`
        )),
        hasMore && /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "flex justify-center pt-4 pb-8", children: /* @__PURE__ */ jsxRuntimeExports.jsx(
          "button",
          {
...
function Settings() {
  ...
  const [cfKey, setCfKey] = reactExports.useState(settings?.curseForgApiKey ?? "");
  ...
  const addPath = async () => {
    ...
    try {
      ...
      if (error || found.length === 0) {
        zt.error(error ?? "No WoW installation found at that path.");
        return;
      }
      for (const inst of found) await addInstallation(inst);
      zt.success(`Added ${found.length} installation(s)`);
      setPathInput("");
    } catch (err) {
  ...
  const autoDetect = async () => {
    ...
    if (!found.length) {
      zt.error("No WoW installations auto-detected");
      return;
    }
    for (const inst of found) await addInstallation(inst);
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex flex-col h-full overflow-y-auto page-enter", children: [
    ...
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex-1 px-6 py-5 space-y-8 max-w-2xl", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsxs(Section, { title: "WoW Installations", children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "space-y-2", children: installations.length === 0 ? /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "text-gray-500 text-sm", children: "No installations configured." }) : installations.map((inst) => /* @__PURE__ */ jsxRuntimeExports.jsx(
          InstallationCard,
          {
            installation: inst,
        ...
        /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex gap-2 mt-3", children: [
          /* @__PURE__ */ jsxRuntimeExports.jsx(
            "input",
            {
              ...
              value: pathInput,
              onChange: (e2) => setPathInput(e2.target.value)
            }
          ),
          /* @__PURE__ */ jsxRuntimeExports.jsx("button", { className: "btn-secondary text-sm", onClick: browseWow, children: "Browse…" }),
          /* @__PURE__ */ jsxRuntimeExports.jsx(
            "button",
            {
              ...
              children: validating ? "Validating…" : "Add"
            }
          )
        ] }),
        ...
        /* @__PURE__ */ jsxRuntimeExports.jsxs("p", { className: "text-gray-500 text-xs mb-3", children: [
          ...
          /* @__PURE__ */ jsxRuntimeExports.jsx(
            "button",
            {
              ...
              children: "console.curseforge.com"
            }
          ),
          "."
        ] }),
        /* @__PURE__ */ jsxRuntimeExports.jsxs("label", { className: "block mb-3", children: [
          /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "section-header", children: "CurseForge API Key" }),
          /* @__PURE__ */ jsxRuntimeExports.jsx(
            "input",
            {
              type: "password",
              ...
              placeholder: "$2a$10$…",
              value: cfKey,
              onChange: (e2) => setCfKey(e2.target.value)
            }
          )
        ] }),
        /* @__PURE__ */ jsxRuntimeExports.jsxs("label", { className: "block mb-4", children: [
          /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "section-header", children: "Wago API Key (optional)" }),
          /* @__PURE__ */ jsxRuntimeExports.jsx(
            "input",
            {
              type: "password",
              ...
              value: wagoKey,
              onChange: (e2) => setWagoKey(e2.target.value)
            }
          )
        ] }),
        /* @__PURE__ */ jsxRuntimeExports.jsx("button", { className: "btn-primary text-sm", onClick: saveKeys, children: "Save Keys" })
      ] }),
      /* @__PURE__ */ jsxRuntimeExports.jsx(Section, { title: "Update Preferences", children: /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "space-y-3", children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx(
          CheckboxRow,
          {
        ...
        /* @__PURE__ */ jsxRuntimeExports.jsxs("label", { className: "flex items-center gap-3 text-sm text-gray-300", children: [
          ...
          /* @__PURE__ */ jsxRuntimeExports.jsxs(
            "select",
            {
              ...
              onChange: (e2) => patchSettings({ defaultChannel: e2.target.value }),
              children: [
                /* @__PURE__ */ jsxRuntimeExports.jsx("option", { value: "stable", children: "Stable only" }),
                /* @__PURE__ */ jsxRuntimeExports.jsx("option", { value: "beta", children: "Stable + Beta" }),
                /* @__PURE__ */ jsxRuntimeExports.jsx("option", { value: "alpha", children: "All (including Alpha)" })
              ]
            }
          )
        ] })
      ] }) }),
      /* @__PURE__ */ jsxRuntimeExports.jsx(Section, { title: "App Behavior", children: /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "space-y-3", children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx(
          CheckboxRow,
          {
            label: "Minimize to system tray on close",
            checked: settings.minimizeToTray,
            onChange: (v2) => patchSettings({ minimizeToTray: v2 })
          }
        ),
        /* @__PURE__ */ jsxRuntimeExports.jsx(
          CheckboxRow,
          {
            ...
            onChange: (v2) => patchSettings({ launchAtLogin: v2 })
          }
        )
      ] }) }),
...
function Section({ title, children }) {
  return /* @__PURE__ */ jsxRuntimeExports.jsxs("section", { children: [
    /* @__PURE__ */ jsxRuntimeExports.jsx("h3", { className: "font-semibold text-gray-200 text-sm mb-3 pb-2 border-b border-gray-800", children: title }),
    children
  ] });
}
function CheckboxRow({
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsxs("label", { className: "flex items-center gap-3 cursor-pointer select-none", children: [
    /* @__PURE__ */ jsxRuntimeExports.jsx(
      "div",
      {
        ...
        children: /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: `absolute top-0.5 w-4 h-4 bg-white rounded-full shadow transition-transform
...
function InstallationCard({
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: `card px-4 py-3 flex items-center gap-3
    ...
    /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "text-xl", children: FLAVOR_ICONS[installation.flavor] ?? "🎮" }),
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex-1 min-w-0", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex items-center gap-2", children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "text-sm font-medium text-gray-200", children: installation.displayName }),
        isActive && /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "badge bg-wow-gold/20 text-wow-gold text-xs", children: "Active" })
      ] }),
      /* @__PURE__ */ jsxRuntimeExports.jsx(
        "p",
        {
          ...
          onClick: onOpenPath,
          children: installation.addonsPath
        }
      ),
      installation.clientVersion && /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "text-gray-700 text-xs", children: installation.clientVersion })
    ] }),
    /* @__PURE__ */ jsxRuntimeExports.jsx(
      "button",
      {
...
function App() {
  return /* @__PURE__ */ jsxRuntimeExports.jsx(AppProvider, { children: /* @__PURE__ */ jsxRuntimeExports.jsxs(HashRouter, { children: [
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex flex-col h-screen overflow-hidden bg-wow-dark", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx(Titlebar, {}),
      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex flex-1 overflow-hidden", children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx(Sidebar, {}),
        /* @__PURE__ */ jsxRuntimeExports.jsx("main", { className: "flex-1 overflow-hidden bg-wow-dark", children: /* @__PURE__ */ jsxRuntimeExports.jsxs(Routes, { children: [
          /* @__PURE__ */ jsxRuntimeExports.jsx(Route, { path: "/", element: /* @__PURE__ */ jsxRuntimeExports.jsx(MyAddons, {}) }),
          /* @__PURE__ */ jsxRuntimeExports.jsx(Route, { path: "/browse", element: /* @__PURE__ */ jsxRuntimeExports.jsx(Browse, {}) }),
